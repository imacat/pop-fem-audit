...
本步驟為**質性閱讀,非編碼**:輸出為自由文字的問題閱讀
報告,無可逐項機械比對的單位,故不適用三票多數決與仲裁;
...
- **5-3 樣態統整**:單筆輸入為 145 份整合報告;歸納問題
  **樣態**——問題呈現與運作的重複形態,非問題分類,
  代表引句僅取自主清單;定義檔
  `prompts/05-03-synthesize.md`,執行一次,歸檔
  `runs/05-03-synthesize/run1`。
